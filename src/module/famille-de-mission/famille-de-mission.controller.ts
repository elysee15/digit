...
export class FamilleDeMissionController {
  ...
  public async getAllFamille() {
    ...
    if (Object.keys(famille).length === 0) {
      return JsonView.dataResponse(
        famille,
        "Mission families empty",
        HttpStatus.OK
      );
    }
    return JsonView.dataResponse(
      famille,
      "Mission families was successfully found",
      HttpStatus.OK
    );
  }
  ...
  public async getFamilleByID(@Param("id", ParseIntPipe) id: number) {
    ...
    return JsonView.dataResponse(
      famille,
      "Mission family was found successfully",
      HttpStatus.OK
    );
  }

  @Post()
  public async postPlaning(
    @Body(new ValidationPipe()) data: FamilleDeMissionEntity
  ) {
    const famille = await this.familleDeMissionService.creating(data);
    if (famille) {
      return JsonView.dataResponse(
        famille,
        "Mission family was save successfully",
        HttpStatus.CREATED
      );
    }
  ...
  public async updatePlaning(
    ...
    const famille = await this.familleDeMissionService.updating(
      planingId,
      data
    );
    return JsonView.dataResponse(
      famille,
      "Mission family was successfully updated",
      HttpStatus.OK
    );
  }
  ...
  public async deleteFamille(@Param("id", ParseIntPipe) id: number) {
    ...
    return JsonView.dataResponse(
      famille,
      "Mission family was successfully deleted"
    );
  }
}
