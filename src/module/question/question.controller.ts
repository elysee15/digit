...
export class QuestionController {
    ...
    public async getAllQuestion(){
        ...
        return JsonView.dataResponse(question, "Liste des questions", HttpStatus.OK);
    }
    ...
    public async postQuestion(@Body() questionDto){
        ...
        if (question){
            return JsonView.dataResponse(question, "La question a été enregistrée avec succès", HttpStatus.OK);
        }
        throw new HttpException("Enregistrement impossible, veuillez réessayer", HttpStatus.NOT_MODIFIED);
    }

    @Put(':questionId')
    public async updateQuestion(@Param('questionId', ParseIntPipe) questionId: number, @Body() questionDto: QuestionEntity){
        const question = await this.questionService.updating(questionId, questionDto);
        if (question){
            return JsonView.dataResponse(question, "La question à été modifiée avec succès", HttpStatus.OK);
        }
        throw new HttpException("Modification impossible car question inexistante", HttpStatus.NOT_FOUND);
    }

    @Delete(':questionId')
    public async deleteQuestion(@Param('questionId', ParseIntPipe) questionId: number){
        const question = await this.questionService.deleting(questionId);
        if (question){
            return "Question supprimée";
        }
        throw new HttpException("Suppression impossible car question inexistante", HttpStatus.NOT_FOUND);
    }
