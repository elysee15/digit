...
export class ProspectionService {
    constructor(
        private readonly prospectionRepository: ProspectionRepository,
    ) {}
    ...
    async getById(prospectionId: number) {
        const prospection = await this.prospectionRepository.findById(prospectionId);
        if (prospection) {
            return prospection;
        }
        return null;
    ...
    async deleting(prospectionId: number) {
        ...
        if (prospection) {
            try {
                ...
                throw new InternalServerErrorException('Impossible de supprimer car cette prospection est en cours d\'utilisation');
            }
        }
        return null;
    }
